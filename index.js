...
const findBestFilms = document.querySelector(".directors-best-films");
...
directors.forEach((obj) => {
  ...
  newElementTitle.textContent = obj.name;
  const newElementP = document.createElement("p");
  newElementP.textContent = obj.career;
  const newElementA = document.createElement("a");
  newElementA.textContent = "Фильмография";
  newElementA.href = obj.films;
  newElementA.target = "_blank";
  findBody.append(newElementTitle, newElementP, newElementA);
});

const topFilmsList = directors.map((obj) => {
  return obj.top_rated_film;
});
console.log(topFilmsList);

const topFilmsListString = topFilmsList.join(",");
console.log(topFilmsListString);

function addTopFilms() {
  const newElementTitle = document.createElement("h3");
  newElementTitle.textContent = "Лучшие фильмы этих режиссеров";
  const newElementP = document.createElement("p");
  newElementP.textContent = topFilmsListString;
  findBody.append(newElementTitle, newElementP);
}
addTopFilms();
